...
namespace Server
{
    class Program
    {
        ...
        static string Calculated_Requisition_Path = FileLocation + @"\calculated_requisition.xml";
        static string SalesHeader_Path = FileLocation + @"\salesheader.xml";
        static string Calculated_Requisition = FileLocation + @"\calculated_requisition.xml";
        ...
        static bool Calculated_Item_Changed = false;
        static bool Calculated_Sale_Changed = false;
        static bool Parent_Child_Item_Changed = false;
        static bool Calculated_Requisition_Changed = false;
        static bool SalesHeader_Changed = false;
        static void fsw_Changed(object sender, FileSystemEventArgs e)
        {            
            FtpClient ftp = new FtpClient(@"ftp://127.0.0.1/", "dispotool", "7Cf4pf2sF");
            switch (e.Name)
            {
                case "calculated_item.xml": Calculated_Item_Changed = true; break;
                case "calculated_sale.xml": Calculated_Sale_Changed = true; break;
                case "parent_child_item.xml": Parent_Child_Item_Changed = true; break;
                case "calculated_requisition.xml": Parent_Child_Item_Changed = true; break;
                case "salesheader.xml": ftp.upload("salesheader.xml", SalesHeader_Path); SalesHeader_Changed = true; break;
            }
            if (Calculated_Item_Changed && Calculated_Sale_Changed && Parent_Child_Item_Changed && SalesHeader_Changed)
            {
                ftp.upload("calculated_item.xml", Calculated_Item_Path);
                ftp.upload("calculated_sale.xml", Calculated_Sale_Path);
                ftp.upload("parent_child_item.xml", Parent_Child_Item_Path);
                ftp.upload("calculated_requisition.xml", Calculated_Requisition_Path);
                ftp.upload("salesheader.xml", SalesHeader_Path);

                Helper.LogWrite(ConsoleColor.Green, "New XML files uploaded to FTP-Server");
                Calculated_Item_Changed = Calculated_Sale_Changed = Parent_Child_Item_Changed = SalesHeader_Changed = false;
                NotifyDataUpdate();
            }
        }
        ...
        public static string GetHashOfFiles() 
        {
            ...
            FileStream calculateditem_stream = File.OpenRead(Calculated_Item_Path);
            FileStream calculatedsale_stream = File.OpenRead(Calculated_Sale_Path);
            FileStream parentchilditem_stream = File.OpenRead(Parent_Child_Item_Path);
            FileStream calculated_requisition_stream = File.OpenRead(Calculated_Requisition_Path);
            FileStream salesheader_stream = File.OpenRead(SalesHeader_Path);
            ...
            byte[] calculated_requisition_hash = md5.ComputeHash(calculated_requisition_stream);
            ...
            StringBuilder calculated_requisition_hashstring = new StringBuilder();
            ...
            for (int i = 0; i < calculateditem_hash.Length; i++) 
            {
                ...
                calculated_requisition_hashstring.Append(calculated_requisition_hash[i].ToString("x2"));
                salesheader_hashstring.Append(salesheader_hash[i].ToString("x2"));
            }
            string result = "";
            result = String.Format("Calculated_Item:{0};Calculated_Sale:{1};Parent_Child_Item:{2};Calculated_Requisition:{3};SalesHeader:{4}", calculateditem_hashstring.ToString(), calculatedsale_hashstring.ToString(), parentchilditem_hashstring.ToString(), calculated_requisition_hashstring.ToString(), salesheader_hashstring.ToString());
            return result;
        }
