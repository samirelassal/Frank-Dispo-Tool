...
namespace Frank_SO_Demand_Report
{
    ...
    public class Report
    {
            ...
            for (int i = 0; i < Calculated_Item.Rows.Count; i++)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        calculated_item["Location"] = item["Location"];

                        #region Aktion
                        if (replenishment_system == "Einkauf") 
                        {
                            if (purchase_order < required_quantity)
                                calculated_item["Action"] = "Material bestellen";
                            else if (qty_on_requisition < required_quantity)
                        ...
                        if (replenishment_system == "Fertigungsauftrag")
                        {
                            ...
                            if (GetQuantityofProducable(calculated_item) == 0)
                                calculated_item["Action"] = "Auf Material warten";

                        }
                        #endregion

                        //Rethink this passage-----------------------------------!!!!!!!!!!-----------------------------------
                        ...
                        foreach (DataRow row in sorted_calculated_sale.Rows) 
                        {
                            ...
                            if (count > missing_quantity - inventory)
                            {
                                ...
                                calculated_item["Menge bis Liefertermin"] = row["Required Quantity"];
                                break;
                            }
                        }
                    }
        ...
        {            
            ...
            string replenishment_system = calculated_item["Replenishment System"].ToString();
            if (replenishment_system == "Fertigungsauftrag" && calculated_item["Action"].ToString() == "")
            {
                ...
                foreach (DataRow dr in parent_child_items)
                {
                    ...
                    if (temp < producable)
                    {
                        if (child_replenishment_sytem == "Fertigungsauftrag")
                        {
                            temp = (GetQuantityofProducable(child) + child_inventory) * required_quantity / child_required_quantity;
                        }
                        producable = temp;
                    }
                }
                calculated_item["Action"] = String.Format("{0} x fertigen", producable.ToString());
                return producable;
            }
            else
                return double.Parse(calculated_item["Action"].ToString().Split(' ')[0]);
        }
