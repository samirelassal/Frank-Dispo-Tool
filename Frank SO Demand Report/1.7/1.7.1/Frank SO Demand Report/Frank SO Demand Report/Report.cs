...
namespace Frank_SO_Demand_Report
{
    ...
    public class Report
    {
        ...
        private  void getSalesData()
        {
            ...
            strSQL = "select \"Document No.\", \"No.\", \"Unit of Measure\", \"Outstanding Quantity\", \"Liefertermin frühestens\", \"Line No.\", \"Priorität\" from \"Sales Line\" where not  \"No.\" = ''";
            adapter = new OdbcDataAdapter(strSQL, connection);
            ...
            for (int i = 0; i < Calculated_Item.Rows.Count; i++)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        DataTable priorities = new DataTable();
                        priorities.Columns.Add("No.");
                        priorities.Columns.Add("Priority Letters");
                        priorities.Columns.Add("Priority Value");
                        priorities.Columns.Add("Quantity");
                        foreach(DataRow salesline in SalesLine.Select("[No.] = '" + calculated_item["No."] + "'"))
                        {
                            LettersAndDigits lad = SplitStringIntoLettersAndDigits(salesline["Priorität"].ToString());
                            var itemno = salesline["No."];
                            var quantity = salesline["Outstanding Quantity"];
                            var letters = lad.Letters;
                            var digits = lad.Digits;
                            priorities.Rows.Add(itemno, letters, digits, quantity);
                        }

                        DataView dv_priorities = new DataView(priorities, "", "[Priority Value] ASC", DataViewRowState.CurrentRows);
                        DataTable sorted_priorities = dv_priorities.ToTable();
                        double count = 0;
                        foreach (DataRow row in sorted_priorities.Rows) 
                        {
                            if (double.TryParse(row["Quantity"].ToString(),  out count) && count > inventory)
                            {
                                calculated_item["Nächste Priorität Buchstaben"] = row["Priority Letters"];
                                calculated_item["Nächste Priorität Zahl"] = row["Priority Value"];
