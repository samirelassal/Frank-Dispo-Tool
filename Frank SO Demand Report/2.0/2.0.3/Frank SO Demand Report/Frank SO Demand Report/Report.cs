...
namespace Frank_SO_Demand_Report
{
    ...
    public class Report
    {
        ...
        private void rekursiveItemData(int ParentID, DataRow[] Children, DataRow _salesline, double parentRequiredQuantity)
        {
            ...
            for (int i = 0; i < Children.Length; i++)
            {
                ...
                if (Child["No."].ToString() == "AK3110")
                    Console.Write("");

                double RequiredQuantity = parentRequiredQuantity * (double)Child["Quantity"];
        ...
        private void FillCalculatedItem() 
        {
            for (int i = 0; i < Calculated_Item.Rows.Count; i++)
            {
                ...
                try
                {
                    if ((double)calculated_item["Required Quantity"] - (double)calculated_item["Inventory"] <= 0)
                    {
