...
namespace Frank_SO_Demand_Report
{
    ...
    public class Report
    {
        ...
        public void Start(string connection_string, string export_data_location)
        {
            ...
            {
                DataRow parent_child_item = Parent_Child_Item.FindByNo(salesheader["No."].ToString())[0];
                FillCalculatedItem(null, parent_child_item);
        ...
        private  void getItemData(DataColumn parentcolumn)
        {
            ...
            foreach (DataRow dr in SalesHeader.Rows)
            {
                ...
                rekursiveItemData(ParentID, SalesLine.Select("[Document No.] = '" + dr["No."] + "'"), null, null);
            }
        ...
        private void setupGeneratedTables()
        {
            ...
            //Tatsächlich Fehlende Menge (Required Quantity - Inventory)
            Calculated_Item.Columns.Add(new DataColumn("Actual Missing Quantity") { DataType = typeof(double) });
        ...
        private void rekursiveItemData(int ParentID, DataRow[] Children, DataRow _salesline, double? Parent_Required_Quantity)
        {
                    ...
                    if (PositionRequiredQuantity > 0)
                    {
                        try
                        {
                            ...
                            if (itemprodBomHeader != null && /*(itemRequiredQuantity > 0) && */(itemReplenishmentSystem == "Fertigungsauftrag") && itemprodBomHeader["Status"].ToString() == "1")
                            {
                                ...
                                rekursiveItemData((int)PCI_Current["ID"], ProductionBOMLines, salesline, itemRequiredQuantity);
                            }
                        }
                        catch (Exception ex)
                        {
        ...
        //This is a 2D list (basically a table) that describes, for what items the missing quantity has been checked already
        //Example:
        //0: AK3110 - AK1234
        //1: AK3110 - AE0276
        //-> The missing quantity of AK3110 has been checked for AK1234 and AE0276
        List<object[]> MissingQuantityCheckedFor = new List<object[]>();

        /// <summary>
        /// Fills in the Calculated_Item table by running through Parent_Child_Item rekursively
        /// </summary>
        /// <param name="parent">The parent_child_item datarow above the current one</param>
        /// p<param name="parent_child_item">The current parent_child_item item</param>
        /// <returns>The amount of children of the current datarow</returns>
        private void FillCalculatedItem(DataRow parent, DataRow parent_child_item)
        {
            try
            {
                bool ReportMode = false;
                DataRow calculated_item = Calculated_Item.Rows.Find(parent_child_item["No."]);
                if (calculated_item != null)
                {
                    DataRow item = Item.Rows.Find(calculated_item["No."]);
                    ReportMode = calculated_item["No."].ToString() == "AK3110" || calculated_item["No."].ToString() == "AR0301";
                    if (ReportMode)
                        Helper.LogFileWrite("Item {0}", calculated_item["No."].ToString());
                    ...
                    else
                    {
                        ...
                        double actual_missing_quantity = required_quantity - inventory;
                        double quantity_able_to_produce;
                        string replenishment_system = calculated_item["Replenishment System"].ToString();
                        if (calculated_item["Description"].ToString() != item["Description"].ToString())
                        {
                            ...
                            calculated_item["Actual Missing Quantity"] = actual_missing_quantity;
                        ...
                        {
                            missing_quantity = (double)calculated_item["Missing Quantity"];
                        }
                        if (ReportMode)
                        {
                            Helper.LogFileWrite("Inventory: {0}", inventory.ToString());
                            Helper.LogFileWrite("Required Quantity: {0}", required_quantity.ToString());
                            Helper.LogFileWrite("Missing Quantity: {0}", missing_quantity.ToString());
                        }
                        var calculated_item_parent = Calculated_Item.Rows.Find(parent["No."]);
                        if (parent != null && calculated_item_parent != null && (double)calculated_item_parent["Required Quantity"] > 0 && !MissingQuantityCheckedFor.Exists(x => x[0].ToString() == calculated_item["No."].ToString() && x[1].ToString() == parent["No."].ToString()))
                        {
                            if (ReportMode) Helper.LogFileWrite("Parent: {0} | Required Quantity: {1}", parent["No."].ToString(), calculated_item_parent["Required Quantity"].ToString());
                            try
                            {
                                double subtrahend = (double)calculated_item_parent["Inventory"];
                                calculated_item["Missing Quantity"] = missing_quantity = (double)calculated_item["Missing Quantity"] - subtrahend;
                                if (ReportMode) Helper.LogFileWrite("Missing Quantity: {0}\r\n", missing_quantity.ToString());
                                MissingQuantityCheckedFor.Add(new object[] { parent_child_item["No."], parent["No."] });
                            }
                            catch (Exception ex) { if (ReportMode) Helper.LogFileWrite(ex.ToString()); }
                        }
                        #region Aktion
                ...
                foreach (var child in Parent_Child_Item.GetChildren((int)parent_child_item["ID"]))
                {
                    FillCalculatedItem(parent_child_item, child);
                }
            }
            catch (Exception ex) { Helper.LogFileWrite("Error in FillCalculatedItem at Item {0}:\r\n {1}\r\nFillCalculatedItem continues with next sales header..", parent_child_item["No."].ToString(), ex.ToString()); }
        }
