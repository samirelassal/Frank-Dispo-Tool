...
namespace Frank_SO_Demand_Report
{
    ...
    public class Report
    {
        ...
        private  void getItemData(DataColumn parentcolumn)
        {
            ...
            strSQL = "select \"No.\", \"Description\", \"Description 2\", \"Replenishment System\", \"Base Unit of Measure\", \"Inventory\", \"Qty. on Sales Order\", \"Menge in Materialanforderungen\", \"Mitarbeiter\", \"Inventory Value Zero\", \"Minuten pro Stück\", \"Einkaufskosten\" from Item";
            adapter = new OdbcDataAdapter(strSQL, connection);
        ...
        private void rekursiveItemData(int ParentID, DataRow[] Children, DataRow _salesline, double parentRequiredQuantity, double parentInventory = 0, bool parentInventoryChanged = false)
        {
            ...
            for (int i = 0; i < Children.Length; i++)
            {
                ...
                if (salesline["Document No."].ToString() == "VB16108" || Child["No."].ToString() == "AK3695" || Child["No."].ToString() == "AK0551")
                    Console.Write("");
                ...
                if (itemNo != "")
                {
                    ...
                    try
                    {
                        ...
                        calculated_item["Required Quantity"] = -1 * (double)item["Inventory"];
                    ...
                    if (PositionRequiredQuantity > 0)
                    {
                        try
                        {
                            ...
                            if (itemprodBomHeader != null && (itemRequiredQuantity > 0) && (itemReplenishmentSystem == "Fertigungsauftrag") && itemprodBomHeader["Status"].ToString() == "1")
                            {
                                ...
                                //if (lessthanzerobefore)
                                    rekursiveItemData((int)PCI_Child["ID"], ProductionBOMLines, salesline, RequiredQuantity, parentInventory*(double)Child["Quantity"] * unit_of_measure + itemInventory, parentInventoryChanged);
                                //else
                                //    rekursiveItemData((int)PCI_Child["ID"], ProductionBOMLines, salesline, RequiredQuantity);
        ...
        private void FillCalculatedItem() 
        {
            for (int i = 0; i < Calculated_Item.Rows.Count; i++)
            {
                ...
                try
                {
                    if ((double)calculated_item["Required Quantity"] - (double)calculated_item["Inventory"] - (double)calculated_item["Parent Inventory"] <= 0)
                    {
                    ...
                    else
                    {
                        ...
                        double missing_quantity = required_quantity - inventory - purchase_order + prod_component - prod_order - qty_on_requisition - parent_inventory;
                        ...
                        if (replenishment_system == "Einkauf")
                        {
                            ...
                            else
                            {
                                ...
                                if (qty_on_requisition + purchase_order < required_quantity - inventory)
                        ...
                        calculated_item["Euro Tatsächlich Fehlende Menge"] = (required_quantity - inventory - parent_inventory) * (double)calculated_item["Euro pro Stück"];
                        #region Priority
