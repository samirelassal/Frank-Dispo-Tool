...
namespace Frank_SO_Demand_Report
{
    ...
    public class Report
    {
        ...
        List<string> filter_SalesHeader = new List<string> {  };
        List<string> monitor_item = new List<string> { "AR2454", "AR2455", "AK0437" };
        List<MonitoredValue> monitores = new List<MonitoredValue> {MonitoredValue.ParentInventory };
        ...
        static void Main(string[] args)
        {
            ...
            Helper.LogFileWrite("Started - Connection String: {0} - Developer Mode: {1}", connection_string, DeveloperMode.ToString());
        ...
        private  void getSalesData()
        {
            ...
            strSQL = "select \"Document No.\", \"No.\", \"Unit of Measure Code\", \"Outstanding Quantity\" as Quantity, \"Liefertermin frühestens\", \"Line No.\", \"Priorität\", \"Shipment Date\" from \"Sales Line\" where not  \"No.\" = ''";
            adapter = new OdbcDataAdapter(strSQL, connection);
        ...
        private  void getItemData(DataColumn parentcolumn)
        {
            ...
            if (DeveloperMode && filter_SalesHeader.Count > 0)
            {
                foreach (string str in filter_SalesHeader)
                {
                    DataRow dr = SalesHeader.Rows.Find(str);
                    //cause crash if dr = null on purpose if devmode is false!!
                    if (!DeveloperMode || dr != null)
                    {
        ...
        private  void getPurchaseData()
        {
            ...
            strSQL = "select \"No.\", \"Name\" from Vendor";
            adapter = new OdbcDataAdapter(strSQL, connection);
            adapter.Fill(Vendor);
        ...
        private void setupGeneratedTables()
        {
            ...
            Calculated_Item.Columns.Add(new DataColumn("Nächster Liefertermin bestätigt") { DataType = typeof(DateTime) });
            ...
            Calculated_Item.Columns.Add(new DataColumn("Not Required") { DataType = typeof(bool) });
            ...
            Calculated_Sale.Columns.Add(new DataColumn("Shipment Date") { DataType = typeof(DateTime) });
            ...
            Calculated_Purchase.Columns.Add("Vendor No.");
            Calculated_Purchase.Columns.Add("Vendor Name");
            //Purchase No.
            Calculated_Purchase.Columns.Add("Purchase No.");
        }
        ...
        private void rekursiveItemData(int ParentID, DataRow[] Children, DataRow _salesline, double parentRequiredQuantity, double parentInventory = 0, double parentProdOrder = 0, bool parentInventoryChanged = false, bool ParentProdOrderChanged = false)
        {
            ...
            for (int i = 0; i < Children.Length; i++)
            {
                ...
                bool parentInventoryInitialised = false;
                #endregion
                ...
                if (itemNo != "")
                {
                    ...
                    if (calculated_item == null)
                    {
                        ...
                        try
                        {
                            ...
                            calculated_item["Not Required"] = false;
                    ...
                    {
                        parentInventoryInitialised = ((double)calculated_item["Parent Inventory"] == 0);
                        ...
                        else
                        {
                            ...
                            if (DeveloperMode && monitores.Contains(MonitoredValue.ParentInventory) && monitor_item.Contains(itemNo))
                                Helper.LogFileWrite("Item {0} with parent {1}: ParentInventory changed: {2} -> {3}", itemNo, Parent_Child_Item.GetRow(ParentID)["No."].ToString(), old_value.ToString(), new_value.ToString());
                        }
                    }
                    ...
                    else
                    {
                        try
                        {
                            ...
                            calculated_sale["Shipment Date"] = salesline["Shipment Date"];
                    ...
                    if (PositionRequiredQuantity > 0)
                    {
                        try
                        {
                            ...
                            if (itemprodBomHeader != null /*&& (itemRequiredQuantity > 0)*/ && (itemReplenishmentSystem == "Fertigungsauftrag") && itemprodBomHeader["Status"].ToString() == "1")
                            {
                                ...
                                rekursiveItemData((int)PCI_Child["ID"], ProductionBOMLines, salesline, RequiredQuantity, parentInventory * (double)Child["Quantity"] * unit_of_measure + (parentInventoryInitialised ? itemInventory : 0), parentProdOrder * (double)Child["Quantity"] * unit_of_measure + itemProdOrder, parentInventoryChanged, ParentProdOrderChanged);
                            }
                        }
                        catch (Exception ex)
                        {
        ...
        private  double getQuantityInPurchaseOrder(DataRow item)
        {
            ...
            foreach (DataRow dr in PurchaseLines)
            {
                ...
                string vendor_name = "";
                var vendors = Vendor.Select("[No.] = '" + dr["Buy-from Vendor No."] + "'");
                if (vendors.Length > 0)
                    vendor_name = vendors[0]["Name"].ToString();
                Calculated_Purchase.Rows.Add(dr["No."], dr["Expected Receipt Date"], dr["Buy-from Vendor No."], vendor_name, dr["Document No."]);
            }
            return result;
        }
        ...
        private void FillCalculatedItem() 
        {
            for (int i = 0; i < Calculated_Item.Rows.Count; i++)
            {
                ...
                try
                {
                    ...
                    {
                        //new in version 1.0.2, calculated item was removed in previous versions
                        //calculated_item["Not Required"] = true;
                    ...
                    else
                    {
                        ...
                        foreach (DataRow row in sorted_calculated_sale.Rows)
                        {
                            ...
                            if (count > available_quantity)
                            {
                                ...
                                calculated_item["Menge bis Priorität"] = count - available_quantity;
                                break;
                            }
                        }
                        #endregion
                        #region Shipment Date
                        dv_calculated_sales.Sort = "[Shipment Date] ASC";
                        sorted_calculated_sale = dv_calculated_sales.ToTable();
                        count = 0;
                        foreach (DataRow row in sorted_calculated_sale.Rows)
                        {
                            count += (double)row["Required Quantity"];
                            if (count > available_quantity)
                            {
                                calculated_item["Nächster Liefertermin bestätigt"] = row["Shipment Date"];
                                calculated_item["Menge bis Liefertermin"] = count - available_quantity;
                                break;
                            }
                        }
                        #endregion
            ...
            Helper.LogFileWrite("FillCalculatedItem: done");
        }
        ...
        private double GetQuantityofProducable(DataRow calculated_item)
        {
            ...
            if (replenishment_system == "Fertigungsauftrag" && pbh.Length > 0)
            {
                ...
                foreach (DataRow dr in prodbomlines)
                {
                    ...
                    if (child != null)
                    {
                        double child_availableqty = (double)child["Inventory"];
