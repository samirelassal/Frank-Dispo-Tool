...
namespace Client_Library
{
    public struct Hashes
    {
        ...
        public string Calculated_Requisition_Hash { get; private set; }
        ...
        internal Hashes(string ServerResult) : this()
        {
            ...
            Calculated_Requisition_Hash = segments[2].Split(':')[1];
    ...
    public static class DispoClient
    {
        ...
        public static readonly string Calculated_Requisition_Path = FileLocation + @"\calculated_requisition.xml";
            ...
            if (serverInfo != null)
            {
                if (Helper.CompareVersion(serverInfo.Value.RequiredVersion, ClientVersion))
                {
                    try
                    {
                        ...
                        ftp.download("calculated_requisition.xml", Calculated_Requisition_Path + "_1");
                    ...
                    try
                    {
                        ...
                        if (newhashes.Calculated_Requisition_Hash == serverInfo.Value.Hashes.Calculated_Requisition_Hash)
                            File.Copy(Calculated_Requisition_Path + "_1", Calculated_Requisition_Path, true);
                        else
                            Helper.ErrorMessage(result += Calculated_Requisition_Path + " ist fehlerhaft!");
        ...
        public static Hashes GetHashOfFiles()
        {
            ...
            FileStream calculatedrequisition_stream = File.OpenRead(Calculated_Requisition_Path + "_1");
            ...
            byte[] calculatedrequisition_hash = md5.ComputeHash(calculatedrequisition_stream);
            ...
            StringBuilder calculatedrequisition_hashstring = new StringBuilder();
            ...
            for (int i = 0; i < calculateditem_hash.Length; i++)
            {
                ...
                calculatedrequisition_hashstring.Append(calculatedrequisition_hash[i].ToString("x2"));
                salesheader_hashstring.Append(salesheader_hash[i].ToString("x2"));
            }
            string result = "";
            result = String.Format("Calculated_Item:{0};Calculated_Sale:{1};Parent_Child_Item:{2};Calulated_Requisition{3};SalesHeader:{4}", calculateditem_hashstring.ToString(), calculatedsale_hashstring.ToString(), parentchilditem_hashstring.ToString(), calculatedrequisition_hashstring.ToString(), salesheader_hashstring.ToString());
