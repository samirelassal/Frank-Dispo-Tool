...
namespace Frank_SO_Demand_Report
{
    public class Report_old
    {
        ...
        private Status rekursiveItemData(DataRow parentRow, double RequiredQuantity, DataRow sales_line, string ParentNo)
        {
            ...
            Status itemStatus = Status.Error;
            ...
            if (ParentNo != null)
            {
                try
                {
                    Parent_Child_Item.Rows.Add(ParentNo, itemNo, RequiredQuantity.ToString());
                }
            ...
            itemStatus = TryGetStatusFromString(calculated_item["Status"].ToString());
            #endregion
            ...
            try
            {
                ...
                if (smallerbefore && itemRequiredQuantity > 0)
            ...
            #region Einkauf
            if (itemReplenishmentSystem == "Einkauf")
            {
                if (itemInventory > itemRequiredQuantity)
                    itemStatus = Status.Green;
                else if (itemInventory + itemPurchaseOrder + itemRequisition >= itemRequiredQuantity)
                    itemStatus = Status.Yellow;
                else
                    itemStatus = Status.Red;
            }
            #endregion

            #region Fertigungsauftrag
            else
            {
                ...
                Status subitemStatus = Status.Green;
                ...
                foreach (DataRow dr in ProductionBOMLines)
                {
                    ...
                    Status status = rekursiveItemData(dr, RequiredQuantity * prod_bom_line_qty, sales_line, itemNo);
                    DataRow subitem = Calculated_Item.Rows.Find(dr["No."]);
                    double subitemInventory = 0;
                    double.TryParse(subitem["Inventory"].ToString(), out subitemInventory);

                    #region get the status
                    if (subitemStatus == Status.Green && status == Status.Green)
                        subitemStatus = Status.Green;
                    else if (subitemStatus != Status.Red && subitemInventory > itemRequiredQuantity)
                        subitemStatus = Status.Yellow;
                    else
                        subitemStatus = Status.Red;
                    #endregion
                }
                if (itemRequiredQuantity < itemInventory)
                    itemStatus = Status.Green;
                else
            ...
            calculated_item["Status"] = getStringFromStatus(itemStatus);
            return itemStatus;
        }
