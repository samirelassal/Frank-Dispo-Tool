...
namespace Frank_SO_Demand_Report
{
    ...
    public class Report
    {
        ...
        private  void getSalesData()
        {
            ...
            strSQL = "select \"Document No.\", \"No.\", \"Unit of Measure\", \"Outstanding Quantity\" as Quantity, \"Liefertermin frühestens\", \"Line No.\", \"Priorität\" from \"Sales Line\" where not  \"No.\" = ''";
            adapter = new OdbcDataAdapter(strSQL, connection);
        ...
        private  void getItemData(DataColumn parentcolumn)
        {
            ...
            strSQL = "select \"Production BOM No.\", \"No.\", \"Unit of Measure Code\" as \"Unit of Measure\", \"Quantity\"  from \"Production BOM Line\" where not \"No.\" = ''";
            adapter = new OdbcDataAdapter(strSQL, connection);
        ...
        private void rekursiveItemData(int ParentID, DataRow[] Children, DataRow _salesline)
        {
            ...
            for (int i = 0; i < Children.Length; i++)
            {
                ...
                getUnitFactor(Child["No."], Child["Unit of Measure"], out unit_of_measure);
                ...
                if (itemNo != "")
                {
                    ...
                    if (PositionRequiredQuantity > 0)
                    {
                        try
                        {
                            PCI_Current = Parent_Child_Item.AddChildRow(ParentID, itemNo, Child["Unit of Measure"], PositionRequiredQuantity, Child["Quantity"], (double)calculated_item["Inventory"] >= PositionRequiredQuantity, letters, digits);
        ...
        private  double getQuantityInSalesOrder(DataRow item)
        {
            ...
            {
                double unit_of_measure = 1;
                getUnitFactor(dr["No."], dr["Unit of Measure"], out unit_of_measure);
        ...
        private void FillCalculatedItem() 
        {
            for (int i = 0; i < Calculated_Item.Rows.Count; i++)
            {
                ...
                try
                {
                    ...
                    {
                        double unit_of_measure = 1;
                        getUnitFactor(calculated_item["No."], calculated_item["Base Unit of Measure"], out unit_of_measure);
                        ...
                        double purchase_order = (double)calculated_item["Purchase Order"] * unit_of_measure;
                        double prod_component = getQuantityInComponent(item);
                        double prod_order = getQuantityProdOrder(item);
                        double qty_on_requisition = (double)calculated_item["Qty. on Requisition"] * unit_of_measure;
