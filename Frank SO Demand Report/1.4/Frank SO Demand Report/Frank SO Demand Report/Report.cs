...
namespace Frank_SO_Demand_Report
{
    ...
    public class Report
    {
        ...
        private  void getSalesData()
        {
            ...
            strSQL = "select \"Document Type\", \"No.\", \"Sell-to Customer No.\", \"Sell-to Customer Name\", \"Sell-to Customer Name 2\" from \"Sales Header\" where \"Sales Header\".\"Document Type\" = 1 and Status = 1";
        ...
        private  void getItemData(DataColumn parentcolumn)
        {
            ...
            strSQL = "select \"No.\", \"Item No.\", \"Status\" from \"Production BOM Header\" where \"No.\" LIKE 'SF%'";
            adapter = new OdbcDataAdapter(strSQL, connection);
        ...
        private void rekursiveItemData(DataRow parentRow, double RequiredQuantity, DataRow sales_line, string ParentNo)
        {
            ...
            if (itemNo != "")
            {
                if (sales_line["Document No."].ToString() == "VB15639")
                    Console.Write("");
                ...
                if (ParentNo != null)
                {
                    ...
                    catch (ConstraintException)
                    {
                        ...
                        dr["Quantity"] = double.Parse(dr["Quantity"].ToString()) + RequiredQuantity;
                    }
                }
        ...
        private void setupGeneratedTables()
        {
            ...
            Calculated_Item.Columns.Add("Erforderlich bis Liefertermin");
            Calculated_Item.Columns.Add("Menge bis Liefertermin");
            ...
            Calculated_Sale.Columns.Add("Liefertermin frühestens");
            ...
            for (int i = 0; i < Calculated_Item.Rows.Count; i++)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        dr["Missing Quantity"] = required_quantity - inventory - purchase_order - prod_component - prod_order - qty_on_requisition;
                        ...
                        DataView dv_calculated_sales = new DataView(Calculated_Sale, "[Item No.] = '" + dr["No."] + "'", "[Liefertermin frühestens] ASC", DataViewRowState.CurrentRows);
                        DataTable sorted_calculated_sale = dv_calculated_sales.ToTable();
                        double count = 0;
                        foreach (DataRow row in sorted_calculated_sale.Rows) 
                        {
                            count += double.Parse(row["Required Quantity"].ToString());
                            if (count > missing_quantity - inventory)
                            {
                                dr["Erforderlich bis Liefertermin"] = row["Liefertermin frühestens"];
                                dr["Menge bis Liefertermin"] = row["Required Quantity"];
                                break;
                            }
                        }
                    }
        ...
        private void setParentChildStatus()
        {
            for (int i = 0; i < Parent_Child_Item.Rows.Count; i++)
            {
                ...
                {
                    double missing_quantity = double.Parse(calculated_item["Missing Quantity"].ToString());
                    double inventory = 0;
